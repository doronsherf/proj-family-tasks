  ...
  public today_date:Date= new Date();
	 ...
	 */
		...
		this.newTask = this.frm_bldr.group({
			//del_btn:[false] ,
			//task_numbr:["", Validators.required ],
			task_numbr:["0", Validators.min(1)],
			//task_numbr:[0, Validators.required, Validators.min(1)],
			//task_numbr:["", Validators.required && Validators.min(1)],
			descrptn:["", Validators.required],
			created:["Automatic"],// not 'required': the value is inserted automaticly (this.today_date)		
